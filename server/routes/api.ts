...
apiRouter.get('/stats', async (_req, res) => { try { const [stats, quota] = await Promise.all([storage.getDashboardStats(), storage.getQuota()]); res.json({ success: true, stats, quota, envStatus: { nvidiaConfigured: nvidiaAI.isConfigured(), supabaseConnected: storage.isSupabaseActive(), facebookConnected: facebookService.getStatus().connected } }); } catch (err: any) { res.status(500).json({ success: false, error: err.message }); } });
...
apiRouter.post('/publications/:id/publish-now', async (req, res) => { try { res.json({ success: true, publication: await scheduler.publishNow(req.params.id) }); } catch (err: any) { res.status(500).json({ success: false, error: err.message }); } });
apiRouter.post('/publications/:id/retry', async (req, res) => { try { res.json({ success: true, publication: await scheduler.publishNow(req.params.id) }); } catch (err: any) { res.status(500).json({ success: false, error: err.message }); } });
apiRouter.post('/publications/:id/reschedule', async (req, res) => { try { if (!req.body.scheduled_at) return res.status(400).json({ success: false, error: 'scheduled_at é obrigatório.' }); res.json({ success: true, publication: await scheduler.reschedule(req.params.id, req.body.scheduled_at) }); } catch (err: any) { res.status(500).json({ success: false, error: err.message }); } });
apiRouter.post('/publications/:id/cancel', async (req, res) => { try { res.json({ success: true, publication: await scheduler.cancel(req.params.id) }); } catch (err: any) { res.status(500).json({ success: false, error: err.message }); } });
...
apiRouter.post('/facebook/connect', async (_req, res) => { try { res.json(await facebookService.connectSession()); } catch (err: any) { res.status(500).json({ success: false, error: err.message }); } });
apiRouter.post('/facebook/verify-session', async (_req, res) => { try { const valid = await facebookService.verifySessionWithBrowser(); res.json({ success: valid, status: facebookService.getStatus() }); } catch (err: any) { res.status(500).json({ success: false, error: err.message }); } });
apiRouter.post('/facebook/verify-group', async (req, res) => { try { const settings = await storage.getSettings(); res.json(await facebookService.verifyGroupAccess(req.body.groupUrl || settings.facebook_group_url)); } catch (err: any) { res.status(500).json({ success: false, error: err.message }); } });
apiRouter.post('/facebook/test-publish', async (req, res) => { try { const settings = await storage.getSettings(); res.json(await facebookPublisher.publishTest(req.body.groupUrl || settings.facebook_group_url)); } catch (err: any) { res.status(500).json({ success: false, error: err.message }); } });
