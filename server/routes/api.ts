...
apiRouter.post('/auth/login', async (req, res) => {
  try {
    const result = await authService.login(req.body.email, req.body.password, storage.getSupabaseClient());
...
apiRouter.get('/auth/me', (req, res) => {
  ...
  return res.json({ success: true, authenticated: true, user: { id: session.id, email: session.email, name: session.name, role: session.role } });
});
...
apiRouter.get('/stats', async (_req, res) => {
  try {
    const [stats, quota] = await Promise.all([storage.getDashboardStats(), storage.getQuota()]);
    res.json({ success: true, stats, quota, envStatus: { nvidiaConfigured: nvidiaAI.isConfigured(), supabaseConnected: storage.isSupabaseActive(), facebookConnected: facebookService.getStatus().connected } });
...
apiRouter.get('/products', async (req, res) => {
  try {
    ...
    if (search) products = products.filter(p => p.product_name.toLowerCase().includes(search) || p.brand?.toLowerCase().includes(search) || p.sku?.toLowerCase().includes(search));
...
apiRouter.get('/products/:id', async (req, res) => {
  try {
    ...
    res.json({ success: true, product, priceHistory: await storage.getPriceHistoryForProduct(product.id) });
  } catch (err: any) { res.status(500).json({ success: false, error: err.message }); }
});

apiRouter.get('/publications', async (req, res) => {
  try { res.json({ success: true, publications: await storage.getPublications(req.query.status as any) }); }
  catch (err: any) { res.status(500).json({ success: false, error: err.message }); }
});

apiRouter.post('/publications/generate-copy', async (req, res) => {
  try {
...
apiRouter.post('/publications', async (req, res) => {
  try {
...
apiRouter.post('/publications/:id/publish-now', async (req, res) => {
  try { res.json({ success: true, publication: await scheduler.publishNow(req.params.id) }); }
  catch (err: any) { res.status(500).json({ success: false, error: err.message }); }
});
...
apiRouter.post('/publications/:id/reschedule', async (req, res) => {
  try { if (!req.body.scheduled_at) return res.status(400).json({ success: false, error: 'scheduled_at é obrigatório.' }); res.json({ success: true, publication: await scheduler.reschedule(req.params.id, req.body.scheduled_at) }); }
  catch (err: any) { res.status(500).json({ success: false, error: err.message }); }
});
apiRouter.post('/publications/:id/cancel', async (req, res) => {
  try { res.json({ success: true, publication: await scheduler.cancel(req.params.id) }); }
  catch (err: any) { res.status(500).json({ success: false, error: err.message }); }
});
apiRouter.delete('/publications/:id', async (req, res) => {
  try { res.json({ success: await storage.deletePublication(req.params.id) }); }
  catch (err: any) { res.status(500).json({ success: false, error: err.message }); }
});

apiRouter.post('/scheduler/batch-today', async (req, res) => {
  try { res.json({ success: true, ...(await scheduler.scheduleDailyBatch(req.body.targetDate)) }); }
  catch (err: any) { res.status(500).json({ success: false, error: err.message }); }
});
apiRouter.post('/scheduler/run-due', async (_req, res) => {
  try { res.json({ success: true, processed: await scheduler.checkAndProcessDuePublications() }); }
  catch (err: any) { res.status(500).json({ success: false, error: err.message }); }
});
...
apiRouter.post('/facebook/verify-session', async (_req, res) => {
  try { const valid = await facebookService.verifySessionWithBrowser(); res.json({ success: valid, status: facebookService.getStatus() }); }
  catch (err: any) { res.status(500).json({ success: false, error: err.message }); }
});
apiRouter.post('/facebook/verify-group', async (req, res) => {
  try { const settings = await storage.getSettings(); res.json(await facebookService.verifyGroupAccess(req.body.groupUrl || settings.facebook_group_url)); }
  catch (err: any) { res.status(500).json({ success: false, error: err.message }); }
});
...
apiRouter.get('/settings', async (_req, res) => {
  try {
    res.json({ success: true, settings: await storage.getSettings(), envStatus: { hasNvidiaKey: nvidiaAI.isConfigured(), hasSupabaseUrl: Boolean(process.env.SUPABASE_URL), hasSupabaseKey: Boolean(process.env.SUPABASE_SERVICE_ROLE_KEY), hasSessionSecret: Boolean(process.env.SESSION_SECRET) } });
  } catch (err: any) { res.status(500).json({ success: false, error: err.message }); }
});
apiRouter.put('/settings', async (req, res) => {
  try { res.json({ success: true, settings: await storage.updateSettings(req.body) }); }
  catch (err: any) { res.status(500).json({ success: false, error: err.message }); }
});
