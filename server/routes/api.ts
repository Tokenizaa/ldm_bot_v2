...
apiRouter.get('/facebook/scheduled-count', async (_req, res) => { try { const settings = await storage.getSettings(); res.json({ success: true, ...(await facebookAutomation.getScheduledPostCount(settings.facebook_group_url)) }); } catch (err: any) { res.status(500).json({ success: false, error: err.message }); } });
