...
export class FacebookCopyAgent {
  ...
  /**
   * Validates a persisted copy before it is allowed into the Facebook composer.
   * Old database rows are untrusted because they may have been generated by a
   * previous prompt/version. Invalid rows are regenerated or replaced by the
   * deterministic formatter; the scheduler never publishes the stored value blindly.
   */
  ...
  /**
   * Returns a publication-safe copy. A valid persisted copy is reused; anything
   * malformed, stale, prompt-contaminated or structurally unsafe is regenerated.
   */
  ...
  private normalizeAndValidate(
    ...
    // Every hashtag must be traceable to supplied product data. This prevents
    // generic AI SEO tags from becoming fabricated metadata.
    const sourceNormalized = [productName, brand, category, sku].filter(Boolean).join(' ')
      .normalize('NFD').replace(/[\\u0300-\\u036f]/g, '').toLowerCase();
    for (const tag of hashtags) {
      const token = tag.slice(1).normalize('NFD').replace(/[\\u0300-\\u036f]/g, '').toLowerCase();
      if (!sourceNormalized.includes(token)) return null;
    }
