...
class FacebookAutomationService {
  ...
  private async openComposer(execId:string,page:Page,groupUrl:string){const trigger=await this.waitForComposer(execId,page,groupUrl);try{await trigger.click({timeout:2500});}catch(error:any){this.log(execId,'COMPOSER_RETRY',`click inicial falhou; limpando dialog residual: ${error?.message||error}`,'warn');await page.keyboard.press('Escape').catch(()=>undefined);await page.waitForTimeout(150);const staleDialogs=page.locator("[role='dialog']:visible");const staleCount=await staleDialogs.count().catch(()=>0);if(staleCount){await page.keyboard.press('Escape').catch(()=>undefined);await page.waitForTimeout(150);}await trigger.click({timeout:4000}).catch(async()=>trigger.click({force:true,timeout:2500}));}const dialog=page.locator("div[role='dialog']:visible").filter({has:page.locator('[role="textbox"]')}).last();await dialog.waitFor({state:'visible',timeout:8000});await this.editor(page).waitFor({state:'visible',timeout:8000});}
