...
class FacebookAutomationService {
  ...
  private async setDate(execId: string, page: Page, date: string) {
    ...
    const trigger = scheduleDialog.locator("[aria-label='Abrir seletor de data']:visible").first();
  ...
  async verifyGroup(groupUrl: string) {
    try {
      ...
      return { success: true, groupUrl, accessible: true, url: page.url(), title: await page.title().catch(() => '') };
    } catch (error: any) {
      return { success: false, groupUrl, accessible: false, error: error?.message || String(error) };
    }
  }
  ...
  async getFacebookExistingProductText(groupUrl: string): Promise<string> {
    ...
    const plannerUrl = groupUrl.replace(/\/+$/, '') + '/scheduled_posts';
    await page.goto(plannerUrl, { waitUntil: 'domcontentloaded', timeout: 45000 });
    await page.waitForFunction(() => !!(document.body.innerText || '').trim(), null, { timeout: 10000 }).catch(() => undefined);
    const planner = await page.locator('body').innerText().catch(() => '');

    await page.goto(groupUrl, { waitUntil: 'domcontentloaded', timeout: 45000 });
    await page.waitForFunction(() => !!(document.body.innerText || '').trim(), null, { timeout: 10000 }).catch(() => undefined);
    const group = await page.locator('body').innerText().catch(() => '');

    return (planner + ' ' + group).replace(/\s+/g, ' ').trim();
  }
