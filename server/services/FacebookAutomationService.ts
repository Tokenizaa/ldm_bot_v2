...
class FacebookAutomationService {
  ...
  private composerDialog(page:Page):Locator { return page.locator("div[role='dialog'][aria-label='Criar post']:visible").first(); }
  private editor(page:Page):Locator { return this.composerDialog(page).locator("[role='textbox']").first(); }
  private scheduleDialog(page:Page):Locator { return page.locator("[role='dialog']:visible").filter({has:page.getByRole('combobox',{name:/Abrir seletor de data/})}).first(); }
  ...
  private async closeResidualDialogs(page:Page){for(let i=0;i<4;i++){const dialogs=page.locator("div[role='dialog']:visible");if(!(await dialogs.count().catch(()=>0)))return;await page.keyboard.press('Escape').catch(()=>undefined);await page.waitForTimeout(250);}}
  private async cleanupFailedSchedule(page:Page){await this.closeResidualDialogs(page);const remaining=page.locator("div[role='dialog']:visible");if(await remaining.count().catch(()=>0)){await page.keyboard.press('Escape').catch(()=>undefined);}}

  private async openComposer(execId:string,page:Page,_groupUrl:string){
    const existing=this.composerDialog(page);
    if(await existing.count().catch(()=>0)){await page.keyboard.press('Escape').catch(()=>undefined);await existing.waitFor({state:'hidden',timeout:5000}).catch(()=>undefined);}
    ...
    const textbox=dialog.locator("[role='textbox']").first();
    await textbox.waitFor({state:'attached',timeout:5000});
    await textbox.waitFor({state:'visible',timeout:12000});
    await textbox.click({timeout:this.interactionTimeoutMs});
    this.log(execId,'COMPOSER_READY','dialog canônico aberto e textbox canônico disponível');
  }

  private async waitForEditorStable(page:Page,token:string){const editor=this.editor(page);const started=Date.now();let previous='',stableSince=0;while(Date.now()-started<this.tokenStabilityMaxWaitMs){const current=await editor.textContent().catch(()=> '')||'';if(current!==previous){previous=current;stableSince=Date.now();}else if(current.includes(token)&&stableSince&&Date.now()-stableSince>=this.tokenStabilityWindowMs)return;await page.waitForTimeout(this.tokenStabilityPollMs);}if(!(await editor.textContent().catch(()=> '')||'').includes(token))throw new Error(`FACEBOOK_TOKEN_NOT_RENDERED:${token}`);}
  private async waitForVisibleOptionsToClose(page:Page,timeoutMs:number){const started=Date.now();while(Date.now()-started<timeoutMs){if(await page.locator("[role='option']:visible").count().catch(()=>0)===0)return;await page.waitForTimeout(100);}}
  private async activateToken(execId:string,page:Page,token:string,requireOption:boolean){const options=page.locator("div[role='dialog'][aria-label='Criar post']:visible [role='option']:visible");if(requireOption)await options.first().waitFor({state:'visible',timeout:this.tokenActivationTimeoutMs});await this.waitForEditorStable(page,token);await page.waitForTimeout(this.tokenActivationDelayMs);const editor=this.editor(page);await editor.click({position:{x:4,y:4}});await editor.press('End').catch(()=>undefined);await page.keyboard.press('Enter');if(requireOption)await this.waitForVisibleOptionsToClose(page,3000);this.log(execId,'TOKEN_ACTIVATED',`token=${token} delayMs=${this.tokenActivationDelayMs}`);}
  ...
  private async waitForAffiliatePreview(page:Page,affiliateUrl:string,maxWaitMs=this.previewFastTimeoutMs){const editor=this.editor(page),dialog=this.composerDialog(page),started=Date.now();while(Date.now()-started<maxWaitMs){const text=await editor.textContent().catch(()=> '')||'';const hrefCount=await dialog.locator("a[href*='lojadomecanico'],a[href*='mecanico']").count().catch(()=>0);if(text.includes(affiliateUrl)||hrefCount>0)return true;await page.waitForTimeout(150);}return false;}
  private async generateLinkPreview(execId:string,page:Page,copy:string,affiliateUrl:string){const editor=this.editor(page),finalText=copy.trim()+'\n\n'+affiliateUrl;await editor.click();await editor.fill('');const tokenPattern=/(@todos|#[\p{L}\p{N}_]+)/gu;let last=0;for(const match of finalText.matchAll(tokenPattern)){const index=match.index??0,plain=finalText.slice(last,index);if(plain)await this.typeComposerText(editor,plain);const token=match[0];await editor.pressSequentially(token);await this.activateToken(execId,page,token,token.toLowerCase()==='@todos');last=index+token.length;}const tail=finalText.slice(last);if(tail){await this.typeComposerText(editor,tail);if(tail.trim()===affiliateUrl){this.log(execId,'LINK_TYPED',`url=${affiliateUrl}`);await page.waitForTimeout(500);await editor.press('Space');if(!await this.waitForAffiliatePreview(page,affiliateUrl))throw new Error('FACEBOOK_LINK_PREVIEW_INPUT_FAILED');}}const ready=await page.waitForFunction(url=>{const dialogs=[...document.querySelectorAll("div[role='dialog']")],dialog=dialogs.at(-1);if(!dialog)return false;const textbox=dialog.querySelector('[role="textbox"]'),text=textbox?.textContent||'',links=dialog.querySelectorAll("a[href*='lojadomecanico'],a[href*='mecanico']").length,body=dialog.textContent||'';return text.includes(url)||links>0||(/loja do mecânico|lojadomecanico|mecânico/i.test(body)&&dialog.querySelectorAll('img').length>0);},affiliateUrl,{timeout:this.previewTimeoutMs}).catch(()=>false);if(!ready&&!await this.waitForAffiliatePreview(page,affiliateUrl,this.previewFastTimeoutMs))throw new Error('FACEBOOK_LINK_PREVIEW_INPUT_FAILED');this.log(execId,'LINK_PREVIEW_READY',`url=${affiliateUrl}`);}
  private async openScheduleDirect(execId:string,page:Page){const button=this.composerDialog(page).locator("[aria-label='Programar post']").first();await button.waitFor({state:'visible',timeout:12000});await button.click({timeout:this.interactionTimeoutMs});const dialog=this.scheduleDialog(page);await dialog.waitFor({state:'visible',timeout:12000});}
  private async setDate(execId:string,page:Page,date:string){if(!/^\d{4}-\d{2}-\d{2}$/.test(date))throw new Error('FACEBOOK_DATE_INVALID');const target=new Date(`${date}T12:00:00-03:00`);if(Number.isNaN(target.getTime())||target.getTime()<=Date.now())throw new Error('FACEBOOK_SCHEDULE_IN_PAST');const day=String(target.getDate()),monthLong=target.toLocaleDateString('pt-BR',{month:'long'}),year=String(target.getFullYear()),datePattern=new RegExp(`${day} de ${monthLong} de ${year}`,'i'),scheduleDialog=this.scheduleDialog(page);await scheduleDialog.waitFor({state:'visible',timeout:this.calendarTimeoutMs});const cell=scheduleDialog.getByRole('gridcell',{name:datePattern}).first();await cell.waitFor({state:'visible',timeout:this.calendarTimeoutMs});await cell.click({timeout:this.interactionTimeoutMs});this.log(execId,'DATE_READY',`date=${date} mode=canonical-gridcell-scoped`);}
  private async setTime(execId:string,page:Page,time:string){if(!/^\d{2}:\d{2}$/.test(time))throw new Error('FACEBOOK_TIME_INVALID');const scheduleDialog=this.scheduleDialog(page);await scheduleDialog.waitFor({state:'visible',timeout:this.calendarTimeoutMs});const option=scheduleDialog.locator("[role='option']:visible").filter({hasText:time}).first();await option.waitFor({state:'visible',timeout:this.calendarTimeoutMs});await option.click({timeout:this.interactionTimeoutMs});this.log(execId,'TIME_READY',`time=${time} mode=canonical-option-scoped`);}
