...
export class StorageService {
  ...
  constructor() {
    ...
    if (!url || !key) {
      throw new Error('SUPABASE_URL e SUPABASE_SERVICE_ROLE_KEY são obrigatórios. Storage local não é suportado.');
    }
    this.supabase = createClient(url, key, { auth: { persistSession: false, autoRefreshToken: false } });
    logger.system('Supabase configurado como única fonte de verdade (tabelas operacionais: affiliate_links, posts, system_config).');
  }
  ...
  private mapRowToProduct(row: any): Product {
    return {
      id: String(row.id),
      product_identity_key: String(row.product_identity_key || `sku:${row.id}`),
      ...
      previous_price: row.previous_price != null ? Number(row.previous_price) : undefined,
      lowest_price: row.lowest_price != null ? Number(row.lowest_price) : undefined,
      image_url: row.image_url ? String(row.image_url) : undefined,
      active: Boolean(row.monitored ?? true),
      last_scraped_at: row.last_checked_at || row.created_at || new Date().toISOString(),
      created_at: row.created_at || new Date().toISOString(),
      updated_at: row.last_checked_at || row.created_at || new Date().toISOString()
    };
  }
  ...
  private mapRowToPublication(row: any, product?: Product): Publication {
    return {
      id: String(row.id),
      product_id: String(row.affiliate_link_id || ''),
      product,
      scheduled_at: String(row.scheduled_at),
      status: (row.status || 'draft') as PublicationStatus,
  ...
  async getProducts(filterActive = false): Promise<Product[]> {
    ...
    if (filterActive) {
      query = query.eq('monitored', true);
    }
    const { data, error } = await query;
    if (error) {
      logger.system(`Falha ao consultar affiliate_links: ${error.message}`, 'error');
      throw new Error(`Falha ao consultar produtos no Supabase: ${error.message}`);
    }
  ...
  async addPriceHistory(entry: Omit<PriceHistory, 'id'>): Promise<PriceHistory> {
    const record = {
      ...
      affiliate_link_id: entry.product_id,
      price: entry.price,
    ...
    return {
      id: String(data.id),
      product_id: String(data.affiliate_link_id),
  ...
  async getPriceHistoryForProduct(productId: string): Promise<PriceHistory[]> {
    const { data, error } = await this.supabase
      .from('affiliate_price_history')
      .select('*')
      .eq('affiliate_link_id', productId)
  ...
  async createPublication(pub: Omit<Publication, 'id' | 'created_at' | 'updated_at'>): Promise<Publication> {
    if (!pub.content?.includes('/20889')) {
      throw new Error('Publicação recusada: copy sem link afiliado /20889.');
    }
    ...
    const record: Record<string, any> = {
      id: newId,
      group_id: pub.facebook_group_url || settings.facebook_group_url || '',
      group_name: 'Grupo Facebook',
      content: pub.content,
      link: pub.product?.affiliate_url || '',
      image_url: pub.product?.image_url || null,
    ...
    if (error) throw new Error(`Falha ao salvar publicação em posts: ${error.message}`);
    const product = pub.product_id ? await this.getProductById(pub.product_id) : undefined;
    return this.mapRowToPublication(data, product);
  }
  ...
  async updatePublication(id: string, updates: Partial<Publication>): Promise<Publication | undefined> {
    ...
    if (updates.facebook_post_url !== undefined) payload.facebook_post_id = updates.facebook_post_url;
    ...
    if (error) throw new Error(`Falha ao atualizar publicação em posts: ${error.message}`);
    if (!data) return undefined;
  ...
  async deletePublication(id: string): Promise<boolean> {
    ...
    if (error) throw new Error(`Falha ao excluir publicação no Supabase: ${error.message}`);
    return true;
  }
  ...
  async getSettings(): Promise<AppSettings> {
    ...
    // Fallback: check key 'main' if existing in system_config
    const { data: mainData } = await this.supabase
      .from('system_config')
      .select('config')
      .eq('key', 'main')
      .maybeSingle();

    const mainCfg = mainData?.config as any;
    return {
      ...DEFAULT_SETTINGS,
      facebook_group_url: process.env.FACEBOOK_GROUP_URL || mainCfg?.facebook?.activeGroups?.[0]?.url || DEFAULT_SETTINGS.facebook_group_url,
      daily_limit: mainCfg?.facebook?.postsPerDay || DEFAULT_SETTINGS.daily_limit,
      monthly_limit: DEFAULT_SETTINGS.monthly_limit,
      nvidia_model: process.env.NVIDIA_MODEL || DEFAULT_SETTINGS.nvidia_model
    };
  }
  ...
  async getQuota(): Promise<OperationalQuota> {
    ...
    // Regra obrigatória: slots reservados (scheduled, publishing, published) consomem cota
    const monthlyReserved = publications.filter(p =>
      ['scheduled', 'publishing', 'published'].includes(p.status) &&
      (p.published_at || p.scheduled_at).startsWith(currentMonth)
    ).length;

    const dailyReserved = publications.filter(p =>
      ['scheduled', 'publishing', 'published'].includes(p.status) &&
      (p.published_at || p.scheduled_at).startsWith(todayDate)
    ).length;

    return {
      current_month: currentMonth,
      monthly_publication_count: monthlyReserved,
      monthly_limit: settings.monthly_limit,
      daily_publication_count: dailyReserved,
      daily_limit: settings.daily_limit,
      remaining_month: Math.max(0, settings.monthly_limit - monthlyReserved),
      today_date: todayDate
    };
  }

  async getDashboardStats(): Promise<DashboardStats> {
    const [products, publications, quota] = await Promise.all([
      this.getProducts(),
    ...
    return {
      ...
      products_valid: products.filter(p => Boolean(p.product_name && p.original_url && p.current_price > 0 && p.affiliate_url?.endsWith('/20889'))).length,
