...
import { facebookPublisherV2 } from './FacebookPublisherServiceV2.js';
...
export class SchedulerService {
  async scheduleDailyBatch(targetDateStr?: string): Promise<{ scheduled: Publication[]; quota: OperationalQuota; message: string }> {
    ...
    const candidates = products.filter(p => !usedProductIds.has(p.id) && p.current_price > 0 && p.product_name && p.original_url && /^https?:\/\//i.test(p.affiliate_url) && p.affiliate_url.includes('/20889') && p.facebook_copy?.trim() && !/https?:\/\//i.test(p.facebook_copy) && !/R\$/i.test(p.facebook_copy));
    ...
    for (const [index, product] of candidates.slice(0, Math.min(slotsAvailable, availableHours.length)).entries()) {
      const time = availableHours[index];
      if (!time) continue;
      ...
      const publication = await storage.createPublication({ product_id: product.id, scheduled_at: localDate.toISOString(), status: 'scheduled', content: product.facebook_copy!.trim(), facebook_group_url: settings.facebook_group_url });
      const result = await facebookPublisherV2.schedule({
      ...
      if (result.success) {
        scheduled.push(await storage.updatePublication(publication.id, { status: 'scheduled', scheduled_at: result.scheduledAt || localDate.toISOString(), error_message: undefined }) as Publication);
      } else {
    ...
    return { scheduled, quota: await storage.getQuota(), message: `${scheduled.length} publicação(ões) enviadas ao agendamento nativo do Facebook.` };
  }
  ...
  async schedulePublication(publicationId: string): Promise<Publication | undefined> {
    ...
    const result = await facebookPublisherV2.schedule({ groupUrl: pub.facebook_group_url || settings.facebook_group_url, content: pub.content, affiliateUrl: pub.product.affiliate_url, scheduledDate: date, scheduledTime: time });

    if (!result.success) {
      const maxAttempts = pub.max_attempts || 3;
      if (attempts < maxAttempts) {
        const backoffMinutes = Math.min(60, 5 * Math.pow(2, attempts - 1));
        return storage.updatePublication(pub.id, { status: 'scheduled', attempts, max_attempts: maxAttempts, next_attempt_at: new Date(Date.now() + backoffMinutes * 60000).toISOString(), error_message: result.error || 'Falha temporária no Facebook.' });
      }
  ...
  /** Explicit endpoint only. Never runs automatically when the server starts. */
  async checkAndProcessDuePublications(): Promise<number> {
    const now = Date.now();
    const due = (await storage.getPublications('scheduled')).filter(p => p.next_attempt_at && new Date(p.next_attempt_at).getTime() <= now && new Date(p.scheduled_at).getTime() > now);
    let processed = 0;
