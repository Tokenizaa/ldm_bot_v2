...
export class RuntimeSchedulerService {
  ...
  private async pace(): Promise<void> {
    if (!this.lastScheduleOperationAt) {
  ...
  private quota(settings: Awaited<ReturnType<typeof storage.getSettings>>, targetDate: Date): OperationalQuota {
    ...
    return {
      current_month: month,
      monthly_publication_count: monthCount,
      monthly_limit: settings.monthly_limit,
      daily_publication_count: dayCount,
      daily_limit: settings.daily_limit,
      remaining_month: Math.max(0, settings.monthly_limit - monthCount),
      today_date: today,
    };
  }
  ...
  async ensureMonthlySchedule(targetDateStr?: string): Promise<{ scheduled: Publication[]; quota: OperationalQuota; message: string }> {
    return this.withLock(async () => {
      ...
      const products = await this.timedStage('storage.getProducts', async () => (await storage.getProducts(true)).filter(p => p.current_price > 0 && Boolean(p.product_name) && /^https?:\/\//i.test(p.original_url) && /^https?:\/\//i.test(p.affiliate_url) && p.affiliate_url.includes('/20889') && Boolean(p.facebook_copy?.trim())));
      ...
      outer: for (const date of dates) {
        ...
        for (const time of hours) {
          ...
          if (slotIso === new Date(0).toISOString() || new Date(slotIso).getTime() <= Date.now() || this.usedSlots.has(slotIso)) continue;
          ...
          try {
            ...
            if (result.success) {
              ...
              publication.planner_url = result.plannerUrl;
              publication.updated_at = new Date().toISOString();
              failure = result.error || 'FACEBOOK_SCHEDULE_FAILED';
              logger.scheduler(`RUNTIME_SCHEDULE_FAILED product=${product.id} slot=${date}T${time} error=${failure}`, result.uncertain ? 'warn' : 'error');
              break outer;
            }
          } catch (error: any) {
            ...
            publication.updated_at = new Date().toISOString();
            failure = error.message || 'FACEBOOK_SCHEDULE_FAILED';
            logger.scheduler(`RUNTIME_SCHEDULE_ERROR product=${product.id} error=${failure}`, 'error');
            break outer;
          }
        }
      }
      ...
      const message = failure
        ? `Agendamento interrompido por falha do Facebook: ${failure}`
        : `${scheduled.length} agendamento(s) confirmado(s) no Facebook.`;
      logger.scheduler(`[Timing] END ensureMonthlySchedule scheduled=${scheduled.length} failure=${failure || 'none'}`);
  ...
  async schedulePublication(publicationId: string): Promise<Publication | undefined> {
    ...
    await this.timedStage(`pace product=${product.id}`, () => this.pace());
    const result = await this.timedStage(`facebookAutomation.schedule product=${product.id} slot=${date}T${time}`, () => facebookAutomation.schedule({
      groupUrl: publication.facebook_group_url || '',
  ...
  async reconcileUnknownPublication(publicationId: string) {
    ...
    if (check.found) {
      publication.status = 'scheduled';
      publication.planner_url = check.plannerUrl;
      publication.error_message = undefined;
    }
    return publication;
  }
  ...
  async getRuntimePublications(): Promise<Publication[]> {
    return [...this.runtime.values()].map(x => x.publication);
  }
}
