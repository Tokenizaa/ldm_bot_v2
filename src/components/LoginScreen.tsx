    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 transition-colors">
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 sm:p-8 shadow-xl">
          <div className="mb-6">
            <h2 className="text-lg font-semibold">{mode === 'signup' ? 'Criar administrador' : 'Acessar Painel de Controle'}</h2>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
              {mode === 'signup' ? 'Crie o primeiro perfil administrativo do ForgeDeals.' : 'Informe as credenciais da sua conta administrativa.'}
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {mode === 'signup' && (
              <div>
                ...
                <div className="relative">
                  ...
                  <input id="signup-name-input" type="text" required value={name} onChange={(e) => setName(e.target.value)} placeholder="Seu nome" className="w-full pl-10 pr-3.5 py-2.5 text-sm bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500/30 dark:text-white" />
                </div>
              </div>
            )}
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input id="login-email-input" type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="admin@forgedeals.com" className="w-full pl-10 pr-3.5 py-2.5 text-sm bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500/30 dark:text-white" />
              </div>
            </div>
            ...
            <div>
              ...
              {mode === 'signup' && <p className="text-[11px] text-slate-500 mt-1.5">Mínimo de 8 caracteres.</p>}
            </div>

            <button id={mode === 'signup' ? 'signup-submit-btn' : 'login-submit-btn'} type="submit" disabled={loading} className="w-full py-2.5 px-4 bg-amber-500 hover:bg-amber-600 disabled:opacity-50 text-slate-950 font-semibold rounded-xl text-sm flex items-center justify-center gap-2">
              {loading ? <><div className="w-4 h-4 border-2 border-slate-950 border-t-transparent rounded-full animate-spin" /><span>{mode === 'signup' ? 'Criando administrador...' : 'Validando credenciais...'}</span></> : <><span>{mode === 'signup' ? 'Criar perfil de administrador' : 'Entrar no Painel'}</span><ArrowRight className="w-4 h-4" /></>}
            </button>
          </form>
          ...
          <div className="mt-5 flex justify-center">
            {mode === 'login' ? (
              ...
              <button id="show-login-btn" type="button" onClick={() => switchMode('login')} className="text-xs font-semibold text-slate-600 dark:text-slate-300 hover:underline">Já tenho uma conta — voltar ao login</button>
            )}
          </div>
          ...
          <div className="mt-6 pt-5 border-t border-slate-100 dark:border-slate-800 flex items-start gap-2.5 text-[11px] text-slate-500 dark:text-slate-400">
            ...
            <div><span className="font-semibold text-slate-700 dark:text-slate-300">Segurança:</span> o cadastro administrativo é permitido somente para o primeiro usuário do Supabase Auth. Depois disso, o botão de cadastro não cria novos administradores.</div>
          </div>
        </div>
      </div>
    </div>
