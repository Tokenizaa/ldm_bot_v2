...
import { Clock, CheckCircle2, AlertCircle, XCircle, RotateCw, ExternalLink, Eye, Trash2, CalendarCheck, HelpCircle } from 'lucide-react';
...
export const ScheduleTab: React.FC<ScheduleTabProps> = ({ publications, onRetry, onDelete, onViewDetails, onScheduleAll, onPublishNow, isSchedulingAll }) => {
  ...
  const getStatusBadge = (status: PublicationStatus) => {
    switch (status) {
      ...
      case 'unknown': return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold bg-purple-100 dark:bg-purple-950/50 text-purple-800 dark:text-purple-300 border border-purple-300 dark:border-purple-800" title="Clique enviado no Facebook; aguardando confirmação no planner"><HelpCircle className="w-3.5 h-3.5 mr-1 text-purple-600 dark:text-purple-400" />Aguardando Verificação</span>;
    ...
    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xs transition-colors">
      ...
      <div className="flex items-center gap-2.5 w-full sm:w-auto">
        <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className="text-xs bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-3 py-2 text-slate-700 dark:text-slate-300 focus:outline-none focus:ring-2 focus:ring-amber-500/30">
          <option value="all">Todos os status ({publications.length})</option><option value="scheduled">Agendados</option><option value="unknown">Aguardando Verificação</option><option value="published">Publicados</option><option value="failed">Falhas</option><option value="cancelled">Cancelados</option>
        </select>
    ...
    <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-xs transition-colors">
      ...
      <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-xs">
        {filtered.length === 0 ? <tr><td colSpan={5} className="py-12 text-center text-slate-400 dark:text-slate-500">Nenhuma publicação encontrada no filtro selecionado.</td></tr> : filtered.map(pub => {
          const scheduleDate = new Date(pub.scheduled_at); const isPast = scheduleDate < new Date();
          return <tr key={pub.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors">
            ...
            <td className="py-3 px-4 text-right whitespace-nowrap"><div className="inline-flex items-center gap-1.5">
              ...
              {(pub.status === 'draft' || pub.status === 'scheduled' || pub.status === 'failed') && <button onClick={() => onPublishNow(pub.id)} className="px-2.5 py-1 text-xs font-semibold bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 hover:bg-emerald-100 rounded-lg border border-emerald-200 dark:border-emerald-800" title="Publicar agora">Publicar</button>}
              {pub.status === 'unknown' && <button onClick={() => onRetry(pub.id)} className="px-2.5 py-1 text-xs font-semibold bg-purple-50 dark:bg-purple-950/40 text-purple-700 dark:text-purple-300 hover:bg-purple-100 rounded-lg border border-purple-200 dark:border-purple-800 flex items-center gap-1" title="Verificar no Planner do Facebook e reconciliar"><RotateCw className="w-3 h-3" /><span>Verificar Planner</span></button>}
              {pub.status === 'failed' && isRetryable(pub) && <button onClick={() => onRetry(pub.id)} className="px-2.5 py-1 text-xs font-semibold bg-rose-50 dark:bg-rose-950/40 text-rose-700 dark:text-rose-300 hover:bg-rose-100 rounded-lg border border-rose-200 dark:border-rose-800 flex items-center gap-1" title="Tentar novamente"><RotateCw className="w-3 h-3" /><span>Retry</span></button>}
