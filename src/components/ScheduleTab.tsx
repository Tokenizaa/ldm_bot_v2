...
import { Clock, CheckCircle2, AlertCircle, XCircle, RotateCw, ExternalLink, Eye, Trash2, Sparkles, CalendarCheck } from 'lucide-react';
...
export const ScheduleTab: React.FC<ScheduleTabProps> = ({ publications, onPublishNow, onRetry, onCancel, onReschedule, onDelete, onViewDetails, onGenerateTodayBatch, isGeneratingBatch }) => {
  ...
  const handleStartReschedule = (pub: Publication) => {
    ...
    setNewScheduleTime(new Date(date.getTime() - date.getTimezoneOffset() * 60000).toISOString().slice(0, 16));
  };
  const handleSaveReschedule = (id: string) => { if (newScheduleTime) { onReschedule(id, new Date(newScheduleTime).toISOString()); setReschedulingId(null); } };
      ...
      <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-xs">
        {filtered.length === 0 ? <tr><td colSpan={5} className="py-12 text-center text-slate-400 dark:text-slate-500">Nenhuma publicação encontrada no filtro selecionado.</td></tr> : filtered.map(pub => {
          ...
          return <tr key={pub.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors">
            <td className="py-3 px-4 whitespace-nowrap">{isRescheduling ? <div className="flex items-center gap-1.5"><input type="datetime-local" value={newScheduleTime} onChange={e => setNewScheduleTime(e.target.value)} className="p-1 text-xs border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 dark:text-white rounded" /><button onClick={() => handleSaveReschedule(pub.id)} className="px-2 py-1 bg-emerald-600 text-white rounded text-[11px] font-semibold">Salvar</button><button onClick={() => setReschedulingId(null)} className="px-2 py-1 bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 rounded text-[11px]">X</button></div> : <div><div className="flex items-center gap-1.5 font-bold text-slate-800 dark:text-slate-200"><Clock className={`w-3.5 h-3.5 ${isPast && pub.status === 'scheduled' ? 'text-amber-500' : 'text-slate-400'}`} /><span>{scheduleDate.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}</span></div><div className="text-[11px] text-slate-400 dark:text-slate-500 mt-0.5">{scheduleDate.toLocaleDateString('pt-BR')}</div></div>}</td>
